import { RARITY_CONFIG, RARITY_ORDER, ASCENSION_CEILING, GEAR_SLOT, CURRENCY } from '../data/constants.js';
...
class HeroInstance {
  ...
  canLevelUp() {
    return this.xp >= this.xpThreshold() && this.level < this.currentStarLevelCap();
  }
  ...
  canStarUp() {
    return this.level >= this.currentStarLevelCap() && this.stars < RARITY_CONFIG[this.rarity].maxStars;
  }
  ...
  ascend() {
    ...
    const nextIndex = RARITY_ORDER[this.rarity] + 1;
    this.rarity = rarityKeys[nextIndex];
